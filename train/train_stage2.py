...
from utils.utils import set_requires_grad, adjust_learning_rate, \
    load_state_from_model, visualize, eval_seg, save_file, compose_DN_mask
...
parser.add_argument('--stage1_model_name', default='stage1_model_name', help='load model weight and prototypes from stage1 trained model')
...
if args.dataset =='KPdataset':
    ...
    from datasets.stage2_KPD_train import stage2_KP_dataset as dataset
    from datasets.KPD_test import KP_dataset_test
        
elif args.dataset =='MFdataset':
    augmentation_methods = [RandomFlip_MF_fake(prob=0.5)]
    from datasets.stage2_MFD_train import stage2_MF_dataset as dataset
...
# load weights from best model on stage 1
    stage1_model_name = args.stage1_model_name
    stage1_model_file = os.path.join(args.root_dir, args.check_dir, stage1_model_name, stage1_model_name + '_best_model.pth')
    pretrained_weight = torch.load(stage1_model_file, map_location=lambda storage, loc: storage.cuda(0))
    load_state_from_model(pretrained_weight, model)
    
# load prototypes from best model on stage 1
    total_prototype = np.load(os.path.join('checkpoints', stage1_model_name, 'total_prototype.npy'))
    total_prototype = torch.FloatTensor(total_prototype).cuda()
    ...
    for epoch in tqdm.tqdm(range(start_epoch, args.max_epoch)):
        ...
        for i, batch in enumerate(trainloader):
            ...
            day_mask, night_mask = compose_DN_mask(args.dataset, names)
            
            if (day_mask.sum().item()==0) or (night_mask.sum().item()==0):
                print('skip current iteration')
                continue
        ...
        # 1. Encoder/Decoder 
            ...
            pred1_seg_loss_rgb, pixel_loss1 = loss_calc(pred1, labels, day_mask)
            pred2_seg_loss_th, pixel_loss2 = loss_calc(pred2, labels, day_mask)
        ...
        # 2. Masked Mutual Learning          
            ...
            mask_ = F.softmax((-1)*(pixel_loss),dim=2).permute(0,2,1).contiguous().reshape(pixel_loss1.size(0),2,H,W) # inter spectrum 
            
            sig_pixel_loss = (1-F.sigmoid(pixel_loss))*2 # intra spectrum
            mask = mask_*(sig_pixel_loss.permute(0,2,1).contiguous().reshape(pixel_loss1.size(0),2,H,W))
            
            rgb_mask, th_mask = mask[:,0], mask[:,1] 
                
            th_mask = th_mask.unsqueeze(1).expand_as(pred1[day_mask==1]) 
            pred1_kl_loss_rgb = softmax_kl_loss(pred1[day_mask==1], pred2[day_mask==1], th_mask)
            
            rgb_mask = rgb_mask.unsqueeze(1).expand_as(pred2[day_mask==1])
            pred2_kl_loss_th = softmax_kl_loss(pred2[day_mask==1], pred1[day_mask==1], rgb_mask)
            
        # 3. MS Prototypes 
            _, _, prototype_RT, cross_entropy_weight = \
                            make_common_prototype_confidence(0.1, 0.1, ms_pred1[day_mask==1], ms_pred2[day_mask==1], pred1[day_mask==1].clone().detach(), pred2[day_mask==1].clone().detach(), labels[day_mask==1])
            ...
            proto_seg_loss_rgb = commmon_prototype_sim_loss(total_prototype, ms_pred1[day_mask==1], labels[day_mask==1], cross_entropy_weight)
            proto_seg_loss_th = commmon_prototype_sim_loss(total_prototype, ms_pred2[day_mask==1], labels[day_mask==1], cross_entropy_weight)             
            
        # (Only Stage 2) Prototypical Self-supervised Loss    
            th_SSL_loss = loss_calc_night(pred2[night_mask==1], total_prototype, ms_pred2[night_mask==1],)
        
        # Total Loss
            loss_rgb = pred1_seg_loss_rgb + proto_seg_loss_rgb*0.2 + pred1_kl_loss_rgb*kl_weight
            loss_th = pred2_seg_loss_th + proto_seg_loss_th*0.2 + pred2_kl_loss_th*kl_weight + th_SSL_loss*20
            ...
            scalar_info_th = {
                ...
                'G2/stage2_SSL_proto': th_SSL_loss.item()
            }   
            scalar_info_proto = {
