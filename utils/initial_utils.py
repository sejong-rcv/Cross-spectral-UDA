...
def init_pretrained_weights(net, pretrained, net_type, init_type='normal'):
    ...
    if os.path.isfile(pretrained):
        ...
        if net_type.find('encoder') !=(-1):  # rgb encoder
            pretrained_dict={k.replace('encoder_rgb_','module.'):v for k,v in pretrained_dict.items()}
        else: # decoder
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items()
                           if k in model_dict.keys() and 'deconv5.1' not in k}
...
def init_net(net, init_type='normal', net_type = 'encoder_rgb', gpu = None, init_gain=1.0):
